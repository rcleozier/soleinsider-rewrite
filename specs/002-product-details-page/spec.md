...
5. **Given** voting cannot be completed, **When** the visitor selects COP or DROP, **Then** the page preserves the previous vote display and does not block the rest of the product detail experience.
...
- **FR-011**: COP/DROP buttons MUST submit through existing voting behavior and update the visible vote display after a successful vote.
...
- **COP/DROP Vote**: A visitor engagement action and visible demand signal for the product.
...
- **SC-005**: A successful COP or DROP vote updates the visible vote display without requiring a page refresh.
